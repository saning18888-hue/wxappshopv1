...
### v0.1.64 · 2026-08-22 · 商城概况页视觉升级

**范围**：`server/public/admin.html`、`CHANGELOG.md`。

**背景**：用户反馈商城概况页设计较旧，希望按参考截图更新。

**改动**：
- 指标卡片视觉升级：去掉高饱和渐变背景，改为白底卡片 + 左侧图标 + 顶部彩色强调条，数值与标签层级更清晰。
- `metricCard()` 输出结构改为左图右文，同步影响商品分析、网站分析等使用同一组件的页面，实现整体风格统一。
- 日期筛选改为圆角 pill tab，并增加白底卡片容器，视觉更干净。
- 趋势图增加自定义数据类型复选框，可勾选显示/隐藏各指标折线；隐藏 Chart.js 内置图例，避免重复。

**验证**：本地浏览器验证商城概况页新设计加载正常，复选框可切换折线显示。
