...
### v0.1.95 (2026-08-28) fix: 物流配置切换服务商时表单联动失效

- 修复 `updateLogisticsType()` 读取 `shopData.logistics_type`（初始值）导致切换 radio 后快递鸟/快递100 表单区域不会实时显示/隐藏的问题。
- 改为直接读取当前选中的 `input[name="logistics_type"]:checked` 值，切换即时生效。
